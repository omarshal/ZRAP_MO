"!@testing Z_C_PURCHASEDOCUMENTLRP_MO
CLASS ltc_z_c_purchasedocumentlrp_mo
...
RISK LEVEL HARMLESS.
  PRIVATE SECTION.
    ...
    DATA:
      act_results               TYPE STANDARD TABLE OF Z_C_PurchaseDocumentLrp_mo WITH EMPTY KEY,
      lt_z_i_purchasedocumenttp TYPE STANDARD TABLE OF z_i_purchasedoctp_mo WITH EMPTY KEY.

    TYPES zty_float TYPE p LENGTH 10 DECIMALS 2.
    METHODS:
...
CLASS ltc_z_c_purchasedocumentlrp_mo IMPLEMENTATION.

  METHOD class_setup.
    environment = cl_cds_test_environment=>create( i_for_entity = 'Z_C_PURCHASEDOCUMENTLRP_MO' ).
  ENDMETHOD.
  ...
  METHOD price_zero_not_critical.
    ...
    SELECT * FROM z_c_purchasedocumentlrp_mo INTO TABLE @act_results.
